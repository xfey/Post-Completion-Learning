...
@dataclass
class RFTScriptArguments(GRPOScriptArguments):
    """
    Customized Script arguments for RFT training
    """
    # sft_loss_ratio: float = field(
    #     default=2.0,
    #     metadata={"help": "SFT loss相对于GRPO loss的倍率。SFT loss 首先会被归一化到与 GRPO 相同绝对值，然后乘以这个倍率。"}
    # )

    # train_stage1_sft: bool = field(
    #     default=True,
    #     metadata={"help": "是否启用stage 1的SFT loss"}
    # )

    # train_stage1_grpo: bool = field(
    #     default=True,
    #     metadata={"help": "是否启用stage 1的GRPO loss"}
    # )

    # train_stage2_sft: bool = field(
    #     default=True,
    #     metadata={"help": "是否启用stage 2的SFT loss"}
    # )

    stage1_grpo_weight: float = field(
        default=1.0,
        metadata={"help": "stage 1的GRPO loss的权重"}
    )

    stage1_sft_weight: float = field(
        default=0.1,
        metadata={"help": "stage 1的SFT loss的权重"}
    )
    stage2_sft_weight: float = field(
        default=0.1,
        metadata={"help": "stage 2的SFT loss的权重"}
    )
    stage2_warmup_steps: float = field(
        default=0.5,
        metadata={"help": "stage 2 loss warmup使用的周期"}
    )

    using_taer_prompt: str = field(
        default="taer",
        metadata={"help": "使用ta还是taer prompt"}
    )
