...
def format_reward(completions, soft=False, **kwargs):
    """Reward function that checks if the reasoning process is enclosed within <think> and </think> tags, while the final answer is enclosed within <answer> and </answer> tags."""
    pattern = r"^<think>.*?</think>\n?<answer>.*?</answer>"    # note: remove the suffix "$", not ends with answer
    completion_contents = [completion[0]["content"] for completion in completions]
    matches = [re.match(pattern, content, re.DOTALL | re.MULTILINE) for content in completion_contents]
    if soft:
        soft_format_results = [count_formats(c) for c in completion_contents]
        soft_match_results = [0.5 if match else 0 for match in matches]
        return [soft_format_results[i] + soft_match_results[i] for i in range(len(completion_contents))]
    else:
        return [1 if match else 0 for match in matches]



def format_reward_all(completions, soft=False, **kwargs):
    """Reward function that checks if the reasoning process is enclosed within <think> and </think> tags, while the final answer is enclosed within <answer> and </answer> tags."""
...
def count_formats(text: str) -> float:
    count = 0.0
...
def rewarding_reward(completions, solution, **kwargs):
    # reward for rewarding rewards
    accuracy_reward_value = accuracy_reward(completions, solution, **kwargs)
    format_reward_value = format_reward(completions)
    ground_truth = []
    for i in range(len(format_reward_value)):
        result = []
        if accuracy_reward_value[i] is None:
            result.append(0)
        else:
            result.append(accuracy_reward_value[i])
        if format_reward_value[i] is None:
            result.append(0)
        else:
            result.append(format_reward_value[i])
        ground_truth.append(result)
    ## extract completions
    contents = [completion[0]["content"] for completion in completions]
    pattern = r"<reward>.*?</reward>"

    all_rewarding_reward_values = []
    for i,content in enumerate(contents):
        try:
            ...
            if isinstance(results, list) and len(results) > 0:
                ...
                if (not len(results) == len(ground_truth[i])) or (any([v < 0 for v in results])):
                    all_rewarding_reward_values.append(0)
                    continue
                reward_value_matching = []
                for (i_pred_v, i_gt_v) in zip(results, ground_truth[i]):
                    relative_diff = abs(i_gt_v - i_pred_v) / max(i_gt_v, i_pred_v)  # 0-1, smaller is better
                    reward_value_matching.append(relative_diff)
                all_rewarding_reward_values.append(1 - sum(reward_value_matching) / len(reward_value_matching))
            else:
                all_rewarding_reward_values.append(0)
        except:
            all_rewarding_reward_values.append(0)
    return all_rewarding_reward_values


def rewarding_reward_raw(completion, solution, **kwargs):
    # reward for rewarding rewards: for single sample
    if "<evaluation>" in completion:
        thinkanswer_completions = completion[:completion.index('<evaluation>')]
        completions = [[{"content": thinkanswer_completions}]]
    else:
        completions = [[{"content": completion}]]
    accuracy_reward_value = accuracy_reward(completions, [solution], **kwargs)[0]
    format_reward_value = format_reward(completions)[0]

    ground_truth = []
    if accuracy_reward_value is None:
        ground_truth.append(0)
    else:
        if accuracy_reward_value == 1:
            accuracy_reward_value = int(1)
        elif accuracy_reward_value == 0:
            accuracy_reward_value = int(0)
        else:
            # 两位小数
            accuracy_reward_value = round(accuracy_reward_value, 2)
        ground_truth.append(accuracy_reward_value)
    if format_reward_value is None:
        ground_truth.append(0)
    else:
        ground_truth.append(format_reward_value)
    ...
    try:
        ...
        if isinstance(results, list) and len(results) > 0:
            ...
            results = results[len("<reward>"):-len("</reward>")].strip()
            results = eval(results)
            if not len(results) == len(ground_truth):
                return {'valid': False}
            elif any([v < 0 for v in results]):
                return {'valid': False}
            return {'gt': ground_truth, 'dt': results, 'valid': True}
        else:
            return {'valid': False}
    except:
        return {'valid': False}
...
def get_code_format_reward(language: str = "python"):
    ...
    def code_format_reward(completions, **kwargs):
        ...
        matches = [
            re.match(
                rf"^<think>.*?</think><answer>.*?```{sample_language}.*?```.*?</answer>$",
                content,
...
def get_reward_funcs(script_args) -> list[Callable]:
    REWARD_FUNCS_REGISTRY = {
        ...
        "format_all": format_reward_all,
        "rewrew": rewarding_reward,
