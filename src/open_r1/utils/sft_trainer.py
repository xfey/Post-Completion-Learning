...
class SFTTrainer(Trainer):
    ...
    def update_sft_messages_key(self, example):
        """
            [custom] 手动调整了输入的key，避免数据集冲突
            现在可以直接读取 RFT 的数据集，同时用于 RL / SFT 训练
        """
        example["completion"] = example.pop("sft_completion")
        return example
    ...
    def _prepare_dataset(
        ...
        # [custom] update key for SFT
        if "sft_completion" in list(next(iter(dataset)).keys()):
            dataset = dataset.map(self.update_sft_messages_key)
